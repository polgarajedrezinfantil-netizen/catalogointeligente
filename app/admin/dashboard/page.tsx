...
export default async function DashboardPage() {
  ...
  const ganancia = vendidos.reduce((a, p) => a + (Number(p.precio) - Number(p.costo)), 0);
  const invertido = noVendidos.reduce((a, p) => a + Number(p.costo) * Number(p.cantidad), 0);
  ...
  return (
    <div className="max-w-4xl space-y-6">
      <h1 className="font-titulo text-2xl text-durazno">Dashboard</h1>

      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <Tarjeta titulo="Productos" valor={String(productos.length)} />
        <Tarjeta titulo="Vendidos" valor={String(vendidos.length)} sub={`No vendidos: ${noVendidos.length}`} />
        <Tarjeta titulo="Ingresos (vendido)" valor={`${simbolo}${ingresos.toFixed(0)}`} />
        <Tarjeta titulo="Ganancia estimada" valor={`${simbolo}${ganancia.toFixed(0)}`} sub={`Invertido: ${simbolo}${invertido.toFixed(0)}`} />
      </div>
